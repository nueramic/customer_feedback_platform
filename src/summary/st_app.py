...
def cards(shop_option, engine):
    ...
    st.text('Краткое саммари самого популярного отызва: ')
    st.text(d.get('Кратко в целом'))
    st.text('Полный отзыв: ')
    st.text(d.get('Комментарий'))
    st.text('Самый популярный отзыв на персона: ')
    st.text(d.get('Самый популярный отзыв на персона'))
